...
void fillTree(const std::vector<std::vector<float> >& regData,
	      const std::pair<const GBRForestD*,const GBRForestD*>& gbrForests,
	      TreeData& outTreeData,
	      TTree* outTree,
	      unsigned int nrThreads);


void fillFullTree(TTree* inTree, TTree* outTree, TreeData& outTreeData, TFile* outFile, std::string treeName, std::string regOutTag){
...
int main(int argc, char** argv)
{
  ...
  cmdLineInt.addOption("ebName",ebName,"egRegDataEcalHLTV1", " name of the EB tree");
  cmdLineInt.addOption("eeName",eeName,"egRegDataHGCALHLTV1", " name of the EE tree");
  ...
  TTree* inEBTree = HistFuncs::makeChain(ebName,inFilename);
  TTree* inEETree = HistFuncs::makeChain(eeName,inFilename);
  //TTree* evtTree = HistFuncs::makeChain(evtName, inFilename);

  TFile* outFile = new TFile(outFilename,"RECREATE");
  //TTree* outTree = new TTree((std::string(treeName)+"Friend").c_str(),"");
  TTree* outEETree = new TTree((std::string(eeName)+"Friend").c_str(),"");
  TTree* outEBTree = new TTree((std::string(ebName)+"Friend").c_str(),"");

  TreeData outEETreeData;
  TreeData outEBTreeData;

  outEETreeData.createBranches(outEETree);
  outEBTreeData.createBranches(outEBTree);
  ...
  const auto regDataEBAll = HistFuncs::readTree(inEBTree,varsEB+":"+*targetEB,"");
  const auto regDataEEAll = HistFuncs::readTree(inEETree,varsEE+":"+*targetEE,"");
  ...
  // const auto evtData = HistFuncs::readTree(evtTree,"runnr:eventnr:lumiSec:eg_isEB:"+std::string(etBinVar),"");
  // fillTree(regDataEBAll,regDataEEAll,evtData,gbrForests,outTreeData,outTree,nrThreads);
  

  std::cout << "Fill Tree" << std::endl;
  auto& ebforest = gbrForests(0,true);
  auto& eeforest = gbrForests(0,false);


  std::cout << "----------------- EB --------------------" << std::endl;
  fillTree(regDataEBAll,ebforest,outEBTreeData,outEBTree,nrThreads);
  std::cout << "----------------- EE --------------------" << std::endl;
  fillTree(regDataEEAll,eeforest,outEETreeData,outEETree,nrThreads);
  std::cout << "Fill Tree" << std::endl;
  ...
  if(writeFullTree){
    ...
    //(TTree* inTree, TTree* outTree, TreeData& outTreeData, TFile* outFile, std::string treeName, std::string regOutTag)


    fillFullTree(inEBTree, outEBTree, outEBTreeData,outFile,ebName,"EB");
    fillFullTree(inEETree, outEETree, outEETreeData,outFile,eeName,"EE");
    outFile->Write();
    /*
    outFile = new TFile(outFilename,"UPDATE");
    outTree = static_cast<TTree*>(outFile->Get((std::string(treeName)+"Friend").c_str()));
	      ...
	      unsigned int nrThreads)
{
  ...
  auto initThread = [&threads,&regData,&gbrForest,&regFuncs,&threadEntryNrs](unsigned int threadNr){
    const auto entryNr = threadEntryNrs[threadNr];
    if(entryNr<regData.size()){
      threads[threadNr] = std::async(getRes,regData[entryNr],gbrForest);
      return true;
    }else return false;
  };
  ...
  while(nrActiveThreads!=0){
    ...
    for(unsigned int threadNr=0;threadNr<nrActiveThreads;threadNr++){
      ...
      auto entryNr = threadEntryNrs[threadNr];
      if(entryNr%100000==0) std::cout <<"entry "<<entryNr<< "/" <<regData.size()<<" time "<<timer<<std::endl;
      ...
      outTreeData.mean = regResult.first;
      outTreeData.sigma = regResult.second;
      outTreeData.invTar =  1./regData[entryNr].back();
      outTree->Fill();
      threadEntryNrs[threadNr]+=nrThreads;
      if(initThread(threadNr)) newNrActiveThreads = threadNr+1;
    }
...
std::pair<double,double> getRes(const std::vector<float>& regData,const std::pair<const GBRForestD*,const GBRForestD*>& gbrForest)
{
  ...
  [[maybe_unused]] auto bla = regData.data();
